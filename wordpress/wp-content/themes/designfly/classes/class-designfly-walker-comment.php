 ...
 * @since 1.0.0
 */
class DesignFly_Walker_Comment extends Walker_Comment {
	...
	protected function html5_comment( $comment, $depth, $args ) {
		...
		$user = get_userdata($comment->user_id);
			if($comment->user_id>0 && $user->roles[0]=='administrator') :
				
			
		?>
		<<?php echo $tag; ?> id="comment-<?php comment_ID(); ?>-admin" <?php comment_class( $this->has_children ? 'parent admin-comment' : 'admin-comment', $comment);
			 ?>>
			 <?php 
			else: 
			 ?>
			...
			<?php endif; ?>
				<footer class="comment-meta">
					<span class="comment-author vcard">
						...
						// 	}
						// }
						echo "<div class='comment-icon'><img src='".get_template_directory_uri()."/images/comment-logo.png'></div>";
						...
						printf(
							wp_kses(
								...
								__( '%s <span class="screen-reader-text says">said on </span>', 'twentynineteen' ),
								array(
									'span' => array(
										'class' => array(),
									),
							...
							'<b class="fn commenter-name">' . $comment_author . '</b>'
						);
